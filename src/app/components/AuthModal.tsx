...
export default function AuthModal({
  ...
  const oauthLabel = oauthLoading === 'google'
    ? 'Google'
    : oauthLoading === 'github'
      ? 'GitHub'
  ...
  return (
    <div
      ...
      onClick={() => {
        if (hasTypedEmail || isBusy) return;
        onClose();
      }}
    >
      <div className="relative w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <div
          className={[
            'w-full bg-white rounded-[16px] border border-[#EAEAEA] shadow-[0_24px_80px_rgba(0,0,0,0.18)] transition-[filter,opacity] duration-200',
            oauthLoading ? 'blur-[2px] opacity-75' : '',
          ].join(' ')}
        >
          <div className="px-6 py-5 border-b border-[#F0F0F0] flex items-center justify-between">
            ...
            <button
              ...
              disabled={isBusy}
              className="w-8 h-8 rounded-[8px] text-[#9B9B9B] hover:bg-[#F5F5F5] disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center"
            >
              <X size={16} />
            </button>
          </div>
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  e.preventDefault();
                  if (isBusy || !hasTypedEmail) return;
                  void submit();
                }}
                type="email"
                placeholder="Email"
        ...
        {oauthLoading && (
          <div className="absolute inset-0 rounded-[16px] bg-white/45 backdrop-blur-[5px] flex items-center justify-center">
            <div className="rounded-xl border border-white/70 bg-white/84 px-5 py-3 shadow-[0_10px_28px_rgba(0,0,0,0.08)]">
              <p className="text-sm text-[#2E2E2E] loading-shimmer-text">Redirecting to {oauthLabel}...</p>
            </div>
          </div>
        )}
      </div>
